...
try:
    ...
    LIBSUMO_IMPORT_ERROR = None

try:
    import traci
except Exception as exc:
    traci = None
    TRACI_IMPORT_ERROR = exc
else:
    TRACI_IMPORT_ERROR = None
...
try:
    ...
    sumolib = None
    checkBinary = None
    SUMOLIB_IMPORT_ERROR = exc
else:
    SUMOLIB_IMPORT_ERROR = None
...
class TSCSimulator:
    def __init__(self,args, config, port, not_default=True, output_path=None):
        ...
        self.is_libsumo = config.get('is_libsumo', True) and libsumo is not None
        if config.get('is_libsumo', True) and libsumo is None:
            print("[WARN] libsumo is unavailable, falling back to traci. Original error:", LIBSUMO_IMPORT_ERROR)
    ...
    def _init_sim(self, sumocfg_file, seed, episode_length_time, gui=False):
        ...
        if checkBinary is not None:
            binary_path = checkBinary(app)
        else:
            binary_path = shutil.which(app)
            if binary_path is None:
                raise RuntimeError(f"SUMO binary '{app}' was not found in PATH. Please install SUMO and ensure it is available.")
        ...
        if sumocfg_file.split("/")[-1] in ['grid4x4.sumocfg', 'arterial4x4.sumocfg']:
            ...
            command = [
                binary_path, '-c', net_file, '-r', route + '_' + str(self.step_num) + '.trip.xml']
            command += ['-a', "/".join(sumocfg_file.split("/")[:-1] + ['e1.add.xml']) + ", " +
                        "/".join(sumocfg_file.split("/")[:-1] + ['e2.add.xml'])]
        else:
            command = [binary_path, '-c', sumocfg_file]
